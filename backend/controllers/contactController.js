...
exports.getAllMessages = (req, res) => {
  const query = `SELECT * FROM contact_messages ORDER BY created_at DESC`; // Optionally order by the created_at column
  ...
  db.query(query, (err, results) => {
    ...
    res.status(200).json(results); // Send all the messages as response
  });
};
